...
Next, configure the structure using the GUI, and select every block you wish to have in your armor stand. Note the largest size supported by a single structure block is 64x64x64 (without editing your world's NBT data)
...
Next, open your exported structure from earlier using the browse button, or type the path in manually.
![alt text](https://github.com/RavinMaddHatter/Structura/blob/main/docs/browse_file.PNG?raw=true)
Enter a name for your structura pack.
...
The structure will appear around every armor stand in the worlds you load. It is how we can make it work in any world. So get out an armor stand and place it down to see your structure.
![alt text](https://github.com/RavinMaddHatter/Structura/blob/main/docs/example_full.png?raw=true)
You can go through a structure layer by layer if you like by shift right-clicking on the stand. This will minimize all layers except the "active" ones. I cannot add poses without adding a behavior pack so for large structures there will be multiple layers displayed at a time (12 blocks apart)
